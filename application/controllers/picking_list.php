...
class Picking_List extends MY_Controller {
	...
	public function print_list($list_id, $type = 0) {
		...
		$sql = "SELECT c.id,IF(eo.cdrr_id IS NULL,sf.name,ef.name) as facility_name,IF(c.code='D-CDRR',CONCAT('D-CDRR#',c.id),CONCAT('F-CDRR#',c.id)) as cdrr_id
		        FROM cdrr c
		        LEFT JOIN sync_facility sf ON sf.id=c.facility_id
		        LEFT JOIN escm_facility ef ON ef.id=c.facility_id
		        LEFT JOIN facilities f ON f.facilitycode=sf.code
